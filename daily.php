...
// Fetch users with daily payment plan
$dailyQuery = "SELECT full_name, email, totalCost, amountPaid FROM users WHERE paymentPlan = 'Daily_payment'";
$dailyResult = mysqli_query($connection, $dailyQuery);

if ($dailyResult) {
    while ($row = mysqli_fetch_assoc($dailyResult)) {
        // Check if totalCost and amountPaid are not equal
        if ($row['totalCost'] != $row['amountPaid']) {
            sendReminderEmail($row['email'], $row['full_name'], 'Daily');
        }
    }
} else {
