...
// Fetch users with monthly payment plan
$monthlyQuery = "SELECT full_name, email, totalCost, amountPaid FROM users WHERE paymentPlan = 'Monthly_payment'";
...
if ($monthlyResult) {
    while ($row = mysqli_fetch_assoc($monthlyResult)) {
        ...
        if ($row['totalCost'] != $row['amountPaid']) {
            sendReminderEmail($row['email'], $row['full_name'], 'Monthly');
        }
    }
} else {
